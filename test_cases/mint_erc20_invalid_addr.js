const { T721C_CONTRACT_NAME, ZADDRESS } = require('./constants');
const { catToArgs, mintToArgs } = require('./utils');
const {Wallet} = require('ethers');

module.exports = {
    mint_erc20_invalid_addr: async function mint_erc20_invalid_addr() {

        const {accounts, expect} = this;
        const controllers = 'core@1.0.0:esport@1.0.0';

        const {ERC20, Dai} = this.contracts;
        const T721Controller = this.contracts[T721C_CONTRACT_NAME];

        const res = await T721Controller.createGroup(controllers, {from: accounts[0]});
        const id = res.logs[0].args.id;

        const categories = [];

        const sale_start = Math.floor(Date.now() / 1000) + 1000;
        const sale_end = Math.floor(Date.now() / 1000) + 100000;
        const resale_start = Math.floor(Date.now() / 1000) + 1000;
        const resale_end = Math.floor(Date.now() / 1000) + 100000;

        categories.push({
            name: `regular`,
            hierarchy: 'root',
            amount: 100,
            sale_start: sale_start,
            sale_end: sale_end,
            resale_start: resale_start,
            resale_end: resale_end,
            authorization: ZADDRESS,
            attachment: ZADDRESS,
            prices: {
                [ERC20.address]: 100,
                [Dai.address]: 200
            }
        });

        const [nums, addr, byte_data] = catToArgs(categories);
        const gasLimit = (await web3.eth.getBlock("latest")).gasLimit;

        await T721Controller.registerCategories(id, nums, addr, byte_data, {gas: gasLimit});

        const currencies = [
            {
                address: ERC20.address,
                amount: 1000
            }
        ];

        const owners = [
            {
                address: accounts[0],
            },
            {
                address: accounts[1],
            },
            {
                address: accounts[2],
            },
            {
                address: accounts[3],
            },
            {
                address: accounts[4],
            },
            {
                address: accounts[5],
            },
            {
                address: accounts[6],
            },
            {
                address: accounts[7],
            },
            {
                address: accounts[8],
            },
            {
                address: accounts[9],
            },
        ];

        const [mint_nums, mint_addr, mint_sig] = mintToArgs(currencies, owners);
        const err_addr = mint_addr.slice(0, mint_addr.length - 11);

        await ERC20.mint(accounts[0], 100 * 10);
        await ERC20.approve(T721Controller.address, 100 * 10, {from: accounts[0]});
        await expect(T721Controller.verifyMint(id, 0, mint_nums, err_addr, mint_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721C::verifyMintPayment | invalid number of addr arguments');
        return expect(T721Controller.mint(id, 0, mint_nums, err_addr, mint_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721C::processMintPayment | invalid number of addr arguments');
    }
};
