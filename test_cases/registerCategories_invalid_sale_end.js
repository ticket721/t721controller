...
module.exports = {
    registerCategories_invalid_sale_end: async function registerCategories_invalid_sale_end() {
        ...
        const sale_start = Math.floor(Date.now() / 1000) + 100000;
        const sale_end = Math.floor(Date.now() / 1000) + 1000;
        ...
        return expect(T721Controller.registerCategories(id, nums, addr, byte_data, {gas: gasLimit})).to.eventually.be.rejectedWith('T721C::checkTimestamps | sale end is not after sale start');
