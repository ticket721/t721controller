...
module.exports = {
    editCategory_all_but_sale_start: async function editCategory_all_but_sale_start() {
        ...
        expect((await T721Controller.getCategoryPrice(id, 0, ERC20.address)).toNumber()).to.equal(200);
        ...
        await T721Controller.editCategory(id, 0, edit_nums, auth, attachment, hierarchy, prices, currencies);

        recovered = await T721Controller.getCategory(id, 0);

        expect(recovered.amount.toNumber()).to.equal(200);
        expect(recovered.sale_start.toNumber()).to.equal(sale_start);
        expect(recovered.sale_end.toNumber()).to.equal(sale_end + 100);
        expect(recovered.resale_start.toNumber()).to.equal(resale_start + 100);
        expect(recovered.resale_end.toNumber()).to.equal(resale_end + 100);
        expect(recovered.authorization).to.equal(accounts[1]);
        expect(recovered.name).to.equal(strToB32(`regular`));
        expect(recovered.hierarchy).to.equal(strToB32('toor'));
        expect(recovered.sold.toNumber()).to.equal(0);

        expect(recovered.currencies.length).to.equal(2);
        expect(recovered.currencies.indexOf(ERC20.address)).to.not.equal(-1);
        expect(recovered.currencies.indexOf(Dai.address)).to.not.equal(-1);

        expect((await T721Controller.getCategoryPrice(id, 0, ERC20.address)).toNumber()).to.equal(200);
        expect((await T721Controller.getCategoryPrice(id, 0, Dai.address)).toNumber()).to.equal(100);
