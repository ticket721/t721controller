...
module.exports = {
    fixAttachments_invalid_category_idx: async function fixAttachment_invalid_category_idx() {
        ...
        await injectAttachmentSigs(attachments, network_id, T721AttachmentsController.address, id, 1, attachment);
        const [att_names, att_nums, att_addr, att_sig, att_test_nums, att_test_addr] = attachmentToArgs(attachments, id, 1);

        await ERC20.mint(accounts[0], 1300);
        await ERC20.approve(T721AttachmentsController.address, 1300, {from: accounts[0]});
        await expect(T721AttachmentsController.verifyFixAttachments(ticket_id, att_names, att_test_nums, att_test_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::verifyAttachments | invalid category_idx');
        return expect(T721AttachmentsController.fixAttachments(ticket_id, att_names, att_nums, att_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::fixAttachments | invalid category_idx')
