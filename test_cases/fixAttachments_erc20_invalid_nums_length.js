...
module.exports = {
    fixAttachments_erc20_invalid_nums_length: async function fixAttachment_erc20_invalid_nums_length() {
        ...
        const err_nums = att_nums.slice(0, 2);

        await ERC20.mint(accounts[0], 1300);
        await ERC20.approve(T721AttachmentsController.address, 1300, {from: accounts[0]});
        await expect(T721AttachmentsController.verifyAttachments(ticket_id, att_names, err_nums, att_test_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::verifyAttachmentPayment | invalid nums argument count');
        return expect(T721AttachmentsController.fixAttachments(ticket_id, att_names, err_nums, att_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::processAttachmentPayment | invalid nums argument count');
    }
};
