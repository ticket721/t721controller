...
module.exports = {
    getTicketAffiliation: async function getTicketAffiliation() {
        ...
        const affiliation = await T721Controller.getTicketAffiliation(ticket_id);

        expect(affiliation.group_id.toLowerCase()).to.equal(id.toLowerCase());
        expect(affiliation.category_idx.toNumber()).to.equal(0);
