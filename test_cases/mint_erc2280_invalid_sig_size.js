...
module.exports = {
    mint_erc2280_invalid_sig_size: async function mint_erc2280_invalid_sig_size() {
        ...
        const err_sig = mint_sig.slice(0, mint_sig.length - 2);

        await ERC2280.mint(buyer.address, 200 * 10, {from: accounts[0]});
        const T721CE = await getEthersT721CContract(buyer, T721Controller);
        await expect(T721CE.functions.verifyMint(id, 0, mint_nums, mint_addr, err_sig)).to.eventually.be.rejectedWith('T721C::verifyERC2280MintPayment | invalid signature size');
        return expect(T721CE.functions.mint(id, 0, mint_nums, mint_addr, err_sig, {gasLimit: 3000000})).to.eventually.be.rejectedWith('T721C::processERC2280MintPayment | invalid signature size');
