...
module.exports = {
    fixAttachments_erc2280_with_authorization: async function fixAttachment_erc2280_with_authorization() {
        ...
        await ET721AttachmentsController.functions.verifyFixAttachments(ticket_id, att_names, att_test_nums, att_test_addr, att_sig, {from: buyer.address});
        const rec = await ET721AttachmentsController.functions.fixAttachments(ticket_id, att_names, att_nums, att_addr, att_sig);

        let idx = 0;
        let received_sum = 0;
        let fee_sum = 0;
        for (const att of attachments) {
            const fee = (await T721AttachmentsController.getERC2280Fee(ERC2280.address, att.prices[ERC2280.address].price)).toNumber();

            fee_sum += fee;
            received_sum += (att.prices[ERC2280.address].price - fee);

            ++idx;
        }

        expect((await ERC2280.balanceOf(accounts[9])).toNumber()).to.equal(fee_sum);
        expect((await ERC2280.balanceOf(T721AttachmentsController.address)).toNumber()).to.equal(received_sum);
        expect((await T721AttachmentsController.balanceOf(id, ERC2280.address)).toNumber()).to.equal(received_sum);
