...
module.exports = {
    fixAttachments_erc2280_invalid_nums_length: async function fixAttachment_erc2280_invalid_nums_length() {
        ...
        await expect(ET721AttachmentsController.functions.verifyFixAttachments(ticket_id, att_names, err_nums, att_test_addr, att_sig, {from: buyer.address})).to.eventually.be.rejectedWith('T721AC::verifyERC2280AttachmentPayment | invalid nums argument count');
        return expect(ET721AttachmentsController.functions.fixAttachments(ticket_id, att_names, err_nums, att_addr, att_sig, {gasLimit: 3000000})).to.eventually.be.rejectedWith('T721AC::processERC2280AttachmentPayment | invalid nums argument count')
