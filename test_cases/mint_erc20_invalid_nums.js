...
module.exports = {
    mint_erc20_invalid_nums: async function mint_erc20_invalid_nums() {
        ...
        await expect(T721Controller.verifyMint(id, 0, err_nums, mint_addr, mint_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721C::verifyMintPayment | invalid number of nums arguments');
        return expect(T721Controller.mint(id, 0, err_nums, mint_addr, mint_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721C::processMintPayment | invalid number of nums arguments');
    }
};
