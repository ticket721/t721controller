...
module.exports = {
    mint_erc2280_invalid_addr: async function mint_erc2280_invalid_addr() {
        ...
        const err_addr = mint_addr.slice(0, mint_addr.length - 11);

        await ERC2280.mint(buyer.address, 200 * 10, {from: accounts[0]});
        const T721CE = await getEthersT721CContract(buyer, T721Controller);
        await expect(T721CE.functions.verifyMint(id, 0, mint_nums, err_addr, mint_sig)).to.eventually.be.rejectedWith('T721C::verifyERC2280MintPayment | invalid number of addr arguments');
        return expect(T721CE.functions.mint(id, 0, mint_nums, err_addr, mint_sig, {gasLimit: 3000000})).to.eventually.be.rejectedWith('T721C::processERC2280MintPayment | invalid number of addr arguments');
