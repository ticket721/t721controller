...
module.exports = {
    fixAttachments_invalid_auth_sig_length: async function fixAttachment_invalid_auth_sig_length() {
        ...
        const err_sig = att_sig.slice(0, att_sig.length - 130);

        await ERC20.mint(accounts[0], 1300);
        await ERC20.approve(T721AttachmentsController.address, 1300, {from: accounts[0]});
        await expect(T721AttachmentsController.verifyFixAttachments(ticket_id, att_names, att_test_nums, att_test_addr, err_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::verifyAttachments | missing attachment authorization signature');
        return expect(T721AttachmentsController.fixAttachments(ticket_id, att_names, att_nums, att_addr, err_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::fixAttachments | missing attachment authorization signature')
