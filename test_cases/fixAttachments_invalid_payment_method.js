...
module.exports = {
    fixAttachments_invalid_payment_method: async function fixAttachment_invalid_payment_method() {
        ...
        att_test_nums[att_test_nums.length - 2] = 5;
        await expect(T721AttachmentsController.verifyFixAttachments(ticket_id, att_names, att_test_nums, att_test_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::verifyFixAttachments | invalid mode for allowance check');
        att_test_nums[2] = 3; // Change payment method
        att_nums[2] = 3; // Change payment method

        await ERC20.mint(accounts[0], 1300);
        await ERC20.approve(T721AttachmentsController.address, 1300, {from: accounts[0]});
        await expect(T721AttachmentsController.verifyFixAttachments(ticket_id, att_names, att_test_nums, att_test_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::verifyAttachments | unknown payment method');
        return expect(T721AttachmentsController.fixAttachments(ticket_id, att_names, att_nums, att_addr, att_sig, {from: accounts[0]})).to.eventually.be.rejectedWith('T721AC::fixAttachments | unknown payment method')
